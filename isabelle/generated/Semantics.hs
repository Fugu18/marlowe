...
data ApplyAllResult =
  ...
  | ApplyAllNoMatchError | ApplyAllAmbiguousSlotIntervalError;

data ReduceStepResult =
  Reduced ReduceWarning ReduceEffect (SemanticsTypes.State_ext ())
    SemanticsTypes.Contract
  | NotReduced | AmbiguousSlotIntervalReductionError;

data TransactionError = TEAmbiguousSlotIntervalError | TEApplyNoMatchError
...
rem :: Arith.Int -> Arith.Int -> Arith.Int;
rem x y = Arith.minus_int x (Arith.times_int (quot x y) y);

signum :: Arith.Int -> Arith.Int;
signum x =
  (if Arith.less_int Arith.Zero_int x then Arith.one_int
    else (if Arith.equal_int x Arith.Zero_int then Arith.Zero_int
           else Arith.uminus_int Arith.one_int));

quotRem :: Arith.Int -> Arith.Int -> (Arith.Int, Arith.Int);
...
evalValue env state (SemanticsTypes.DivValue lhs rhs) =
  ...
  } in (if Arith.equal_int n Arith.Zero_int then Arith.Zero_int
         else let {
                d = evalValue env state rhs;
              } in (if Arith.equal_int d Arith.Zero_int then Arith.Zero_int
                     else (case quotRem n d of {
                            (q, r) ->
                              let {
                                ar = Arith.times_int (Arith.abs_int r)
                                       (Arith.Pos (Arith.Bit0 Arith.One));
                                ad = Arith.abs_int d;
                              } in (if Arith.less_int ar ad then q
                                     else (if Arith.less_int ad ar
    then Arith.plus_int q (Arith.times_int (signum n) (signum d))
    else (case Arith.equal_int (rem q (Arith.Pos (Arith.Bit0 Arith.One)))
                 Arith.Zero_int
           of {
           True -> q;
           False -> Arith.plus_int q (Arith.times_int (signum n) (signum d));
         })));
                          })));
evalValue env state (SemanticsTypes.Scale n d rhs) =
  let {
    nn = Arith.times_int (evalValue env state rhs) n;
  } in (case quotRem nn d of {
         (q, r) ->
           (if Arith.less_int
                 (Arith.times_int (Arith.abs_int r)
                   (Arith.Pos (Arith.Bit0 Arith.One)))
                 (Arith.abs_int d)
             then q
             else Arith.plus_int q (Arith.times_int (signum nn) (signum d)));
       });
...
evalValue env state SemanticsTypes.SlotIntervalStart =
  fst (SemanticsTypes.slotInterval env);
evalValue env state SemanticsTypes.SlotIntervalEnd =
  snd (SemanticsTypes.slotInterval env);
...
reduceContractStep env state (SemanticsTypes.When uv timeout cont) =
  (case SemanticsTypes.slotInterval env of {
    (startSlot, endSlot) ->
      (if Arith.less_int endSlot timeout then NotReduced
        else (if Arith.less_eq_int timeout startSlot
...
fixInterval (low, high) state =
  let {
    curMinSlot = SemanticsTypes.minSlot state;
    newLow = Orderings.max low curMinSlot;
    curInterval = (newLow, high);
    env = SemanticsTypes.Environment_ext curInterval ();
    newState = SemanticsTypes.minSlot_update (\ _ -> newLow) state;
  } in (if Arith.less_int high low
         then SemanticsTypes.IntervalError
                (SemanticsTypes.InvalidInterval (low, high))
         else (if Arith.less_int high curMinSlot
                then SemanticsTypes.IntervalError
                       (SemanticsTypes.IntervalInPastError curMinSlot
                         (low, high))
         ...
         SemanticsTypes.IntervalTrimmed env fixSta ->
           (case applyAllInputs env fixSta contract inps of {
             ...
             ApplyAllAmbiguousSlotIntervalError ->
               TransactionError TEAmbiguousSlotIntervalError;
           });
